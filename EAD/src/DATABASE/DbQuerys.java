...
//            }
    ...
    public String[] checkADMINLogin(String email, String password) throws SQLException {
        this.connection = connect();
        try {
            ...
            System.out.println("result" + result);
            if (result.next()) {
                String A_ID = result.getString("A_ID");
                System.out.println("Login success" + A_ID);

                System.out.println("Login success");

                return new String[]{"succes", A_ID};
    ...
    public void insertFlight(String name, String airline, String departure, String arrival, String departureTime, String arrivalTime)
    {
        try {
            connection = connect();
        } catch (SQLException e) {
        ...
        try {
            String query = "INSERT INTO Flight (F_Name, F_Airline, DEPARTURE, ARRIVAL, DEPARTUREtime, ARRIVALtime) VALUES (?, ?, ?, ?, ?, ?)";
            pStatement = connection.prepareStatement(query);
            pStatement.setString(1, name);
            pStatement.setString(2, airline);
            pStatement.setString(3, departure);
            pStatement.setString(4, arrival);
            pStatement.setString(5, departureTime);
            pStatement.setString(6, arrivalTime);
            pStatement.executeUpdate();
